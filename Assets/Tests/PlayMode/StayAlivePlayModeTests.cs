...
public sealed class StayAlivePlayModeTests
{
    [UnitySetUp]
    ...
    {
        string storageRoot = Path.Combine(Application.temporaryCachePath, "StayAlivePlayModeSaves");
    ...
    [UnityTest]
    public IEnumerator UxOnboardingHudPromptsBuildAndSaveFeedbackRunInPlayMode()
    {
        string storageRoot = Path.Combine(Application.temporaryCachePath, "StayAliveUxPlayModeSaves");
        if (Directory.Exists(storageRoot))
        {
            Directory.Delete(storageRoot, true);
        }
        ...
        Assert.AreEqual(3, menu.SlotCount);
        Assert.IsFalse(menu.ContinueGame());
        Assert.IsTrue(menu.TitleMenuVisible);
        ...
        station.Interact(player.GetComponent<PlayerInteraction>());
        yield return null;
        Assert.AreEqual(ObjectiveGuideStep.GatherFoodHerbs, guide.CurrentStep);
        Assert.IsNotNull(ui.CurrentRecipeView);

        GatherableResource wood = Object.FindObjectsByType<GatherableResource>(FindObjectsSortMode.None)
            .First(g => g.ResourceType == ResourceType.Wood);
        InteractionPromptData prompt = wood.GetPromptData(player.GetComponent<PlayerInteraction>());
        ui.SetPrompt(prompt);
        Assert.IsTrue(ui.CurrentPrompt.DisplayText.Contains("Gather"));

        resources.Set(ResourceType.Wood, 5);
        BuildPreviewState preview = building.GetPreviewState(BuildableType.Floor, new Vector2(2f, -5f), false);
        ui.ShowBuildPreview(preview);
        Assert.IsTrue(ui.CurrentBuildPreview.DisplayText.Contains("Right Click place"));

        crafted.Set(CraftedItemType.Campfire, 1);
        Assert.IsTrue(building.TryPlace(BuildableType.Campfire, new Vector2(5f, -5f), false, out _));
        Assert.IsTrue(saves.SaveSlot(1));
        Assert.IsTrue(ui.CurrentSaveSlotView.isOccupied);
        ...
        vitals.ApplyDamage(5f);
        Assert.IsTrue(ui.LastMessage.Contains("Took damage"));

        vitals.ApplyDamage(1000f);
        yield return null;
        Assert.IsTrue(menu.DeathMenuVisible);
        Assert.IsTrue(ui.DeathVisible);
        Assert.AreEqual(0f, Time.timeScale);
        Assert.IsTrue(menu.LoadActiveSlot());
        yield return null;
        Assert.IsFalse(menu.DeathMenuVisible);
        Assert.IsFalse(ui.DeathVisible);
        Assert.IsFalse(vitals.IsDead);
        Assert.AreEqual(1f, Time.timeScale);

        player.transform.position = new Vector3(4f, -4f, 0f);
        resources.Set(ResourceType.Wood, 7);
        crafted.Set(CraftedItemType.StoneSpear, 1);
        cycle.SetState(2, 0.9f);
        guide.Evaluate();
        Assert.AreNotEqual(ObjectiveGuideStep.GatherWoodStone, guide.CurrentStep);
        ...
        Assert.AreEqual(vitals.MaxHealth, vitals.Health, 0.05f);
        Assert.AreEqual(vitals.MaxHunger, vitals.Hunger, 0.05f);
        Assert.AreEqual(1, cycle.CurrentDay);
        Assert.AreEqual(ObjectiveGuideStep.GatherWoodStone, guide.CurrentStep);
        Assert.AreEqual(0, building.PlacedBuildables.Count);
    }
}
